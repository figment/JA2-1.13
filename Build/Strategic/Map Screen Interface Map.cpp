...
BOOLEAN PickUpATownPersonFromSector( UINT8 ubType, INT16 sX, INT16 sY )
{
	...
	if (_KeyDown(SHIFT)) countMovedCivs = SectorInfo[ SECTOR( sX, sY )].ubNumberOfCivsAtLevel[ ubType ];
...
DebugMsg (TOPIC_JA2,DBG_LEVEL_3,"Map Screen2");
	...
	switch( ubType )
	{
		case( GREEN_MILITIA ):
			...
			if (_KeyDown(SHIFT)) 
			{
				if (countFreeCivPlace <= sGreensOnCursor) countMovedCivs = countFreeCivPlace;
				else countMovedCivs = sGreensOnCursor;
			}
